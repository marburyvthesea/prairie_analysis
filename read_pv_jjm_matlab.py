...
""" modified from DGalt, Surmeier Lab"""
...
def add_ls_distance_column(ls_df, coordinates):
	"""
	Takes a line scan data frame and adds a column of distance along line from start
	based on "coordinates" variable: linescan "Start X" and "Start Y" points found in xml file
...
def import_folder(folder):
    ...
    if any(vr_xmls):
        ...
        if data_vr:
            output["voltage recording"] = pd.concat(data_vr, axis=1, keys=sweep_list,
                                                    names=['Sweep', 'Index'])
        elif not data_vr:
            output["voltage recording"] = None
        if data_ls:
            output["linescan"] = pd.concat(data_ls, axis=1, keys=sweep_list,
                                           names=['Sweep', 'Index'])
        elif not data_ls:
